...
interface ResolvedStop extends PathStop {
  ...
  resolvedHref: string | null // null = the stop has no page yet
...
export async function generateMetadata({
  ...
  const ogImage = `/api/og/quote?text=${encodeURIComponent(path.arrivalStatement)}&cite=${encodeURIComponent(path.title)}`
  ...
  return {
    ...
    openGraph: {
      ...
      siteName: SITE_NAME,
      type: 'website',
      images: [{ url: ogImage, width: 1200, height: 630, alt: path.title }],
    },
    twitter: {
