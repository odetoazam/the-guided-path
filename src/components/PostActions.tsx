  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const [favRes, progRes] = await Promise.all([
        supabase.from('user_favorites').select('id').eq('user_id', session.user.id).eq('slug', slug).eq('content_type', 'post').maybeSingle(),
        supabase.from('user_progress').select('id').eq('user_id', session.user.id).eq('slug', slug).eq('content_type', 'post').maybeSingle(),
      ])
      setIsFavorited(!!favRes.data)
      const alreadyRead = !!progRes.data
      setIsRead(alreadyRead)
      if (alreadyRead) trackContentReturn(slug, 'post')
    })
      ...
      async (entries) => {
        if (entries[0].isIntersecting && loggedIn && !isRead) {
          ...
          const res = await fetch('/api/progress', {
            ...
            body: JSON.stringify({ slug, type: 'post' }),
          })
        ...
        {loggedIn ? (
          <>
            <button
              ...
              disabled={favLoading}
              title={isFavorited ? 'Remove from saved' : 'Save this reflection'}
              className={[
          ...
          <Link
            href={`/auth/sign-in?next=/posts/${slug}`}
